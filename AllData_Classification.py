...
def main():
    ...
    Nodularity_ISO = []
    if Col == 9:
        model_path = "C:/Data/AllData_final_lda_model_9columns"  # モデルファイル名
    elif Col == 3:
        model_path = "C:/Data/AllData_final_lightgbm_model_3columns"  # モデルファイル名
    ...
    for filename in filenames:
        ...
        if Col == 9:
            result_ISO_filename = (
                src[:idx]
                + "_nodularity(Machine Learning)_AllData_9columns."
                + src[idx + 1 :]
            )
        elif Col == 3:
            result_ISO_filename = (
                src[:idx]
                + "_nodularity(Machine Learning)_AllData_3columns."
                + src[idx + 1 :]
            )
    ...
    if Col == 9:
        f2 = open(
            ...
            + "/result_{0:%Y%m%d%H%M}_nodularity(Machine Learning)_AllData_9columns".format(
                now
            )
            + ".csv",
            "w",
        ...
        f2 = open(
            str(os.path.dirname(filenames[0]))
            + "/result_{0:%Y%m%d%H%M}_nodularity(Machine Learning)_AllData_3columns".format(
                now
            )
            + ".csv",
            "w",
